...
scene = uavScenario(ReferenceLocation=[59.404476 17.946527 0]);

xlimit = [-300 300];
ylimit = [-300 300];

addMesh(scene,"buildings",{"map.osm",xlimit,ylimit,"auto"},[0.6 0.6 0.6]);

scatterers = [];

wallSpacing = 3;   % meters
heightSpacing = 3;
...
for k = 1:numel(scene.Meshes)
    ...
    for e = 1:size(edges,1)
        ...
        npts = ceil(L/wallSpacing);

        t = linspace(0,1,npts)';

        edgePoints = v1 + t.*(v2-v1);

        scatterers = [scatterers; edgePoints];
...
for i = 1:size(scatterers,1)
    ...
    zvals = 0:heightSpacing:15;   % building height estimate

    pts = [ ...
        repmat(base(1),length(zvals),1), ...
        repmat(base(2),length(zvals),1), ...
        zvals' ];

    wallPoints = [wallPoints; pts];
...
points(:,1) = points(:,1) + 900;  % range offset
points(:,2) = points(:,2) - 10;   % cross-range offset

% % Testing a building mesh
% % 
% % Building parameters
% x0 = 900;      % center range
% y0 = -10;      % center cross-range
% width = 5;    % building width (cross-range)
% depth = 10;    % building depth (range)
% height = 10;   % building height
% spacing = 3;   % point spacing
% 
% % Grid coordinates
% x = x0 + (-depth/2:spacing:depth/2);
% y = y0 + (-width/2:spacing:width/2);
% z = 0:spacing:height;
% 
% [X,Y] = meshgrid(x,y);
% 
% % Roof
% roof = [X(:), Y(:), height*ones(numel(X),1)];
% 
% % Ground footprint
% ground = [X(:), Y(:), zeros(numel(X),1)];
% 
% % Walls
% [Xw,Zw] = meshgrid(x,z);
% wall1 = [Xw(:), (y0-width/2)*ones(numel(Xw),1), Zw(:)];
% wall2 = [Xw(:), (y0+width/2)*ones(numel(Xw),1), Zw(:)];
% 
% [Yw,Zw] = meshgrid(y,z);
% wall3 = [(x0-depth/2)*ones(numel(Yw),1), Yw(:), Zw(:)];
% wall4 = [(x0+depth/2)*ones(numel(Yw),1), Yw(:), Zw(:)];
% 
% % Combine all points
% points = [roof; ground; wall1; wall2; wall3; wall4];
% 
...
figure(1)
scatter3(points(:,2),points(:,1),points(:,3),5,'filled')
axis equal
grid on
xlabel('Cross-range')
ylabel('Range')
zlabel('Height')
title('OSM Point Cloud')

% figure(1)
% scatter3(targetpos(2,:), targetpos(1,:), targetpos(3,:), 40, 'filled')
% 
% xlabel('Cross-Range (m)')
% ylabel('Range (m)')
% zlabel('Height (m)')
% title('Ground Truth Targets (3D)')
% 
% grid on
% axis equal
% view(3)
% 
% hold on
% plot3(-600,0,500,'ro','MarkerSize',10,'LineWidth',2)
% legend('Targets','Radar')
...
for ii = 1:numpulses
    ...
    for k = 1:size(targetpos,2)
        ...
        % Ray tracing
        %fprintf('Raytracing iteration %d', k)
        %fprintf('out of %d \n', size(targetpos, 2))
